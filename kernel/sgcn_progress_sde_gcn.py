...
from kernel.evolvegcnh import *

class SGCN_EvolvedGCN_SDE(torch.nn.Module):
    ...
    def __init__(self, graphODE_Model, num_layers, hidden, numofTimepoints, topk_ratio=0.3, hidden_linear=64, rois=100,
        ...
        super(SGCN_EvolvedGCN_SDE, self).__init__()
        ...
        ratio_oftopk = 1
        #build sde inside this module
        self.recurrent = EvolveGCNH(num_of_nodes=int(num_features/ratio_oftopk), in_channels=num_features, gcn_hidden=hidden, device=device, isHiddenVersion=isHiddenVersion4Evolved,
                                    isUseSDE = args.isEvolvedWeightSDE, coeff_for_tps=args.coeff_for_tps, isReturnGrad=isReturnGrad).to(device)
    ...
    def vec_grad(self):
        if self.GRUEnd_layer=="GRUCell":
            ...
            return self.recurrent.vec_grad()
    ...
    def learn_graphs_embedding(self, recons_signal, x, edge_index, edge_weight, isExplain=False, tp_diff_prev=None):
        ...
        h = self.recurrent(x_prob, edge_index, edge_weight_prob, tp_diff_prev)
        x = F.relu(h)
    ...
    def forward(self, recons_signal, adj, conver_sig_len_index, build_graph_bycorr=False, isExplain=False, tp_diff_prev=None, args=None):
        ...
        for cur_tp in range(self.numofTimepoints):
            ...
            out_x, _ = self.learn_graphs_embedding(recons_signal[:,cur_tp,:,:], input_x, edge_index, edge_weight, isExplain=isExplain, tp_diff_prev=tp_diff_prev[:, cur_tp])
