...
class Encoder(torch.nn.Module):
    def __init__(self, hidden_dim_cnn = 5, k_neig = 5, training_len=140, num_nodes = 100, hidden_gcn = 64, out_dim=16, topk_ratio=0.3, device = None):
        super(Encoder, self).__init__()
        self.cnn = nn.Conv1d(1, hidden_dim_cnn, 3, stride=1, padding=1)
...
class LatentGraphODE(torch.nn.Module):
    def __init__(self, hidden_dim_cnn=5, hidden_gcn = 64, out_dim=16, topk_ratio=0.3, len_of_pred_sig = 140, training_len=100, rois=100, z0_prior=None, obsrv_std=0.01, device = None):
        super(LatentGraphODE, self).__init__()
        self.device=device
        self.z0_prior=z0_prior
        self.obsrv_std=obsrv_std
        self.out_dim=out_dim
        self.encoder = Encoder(hidden_dim_cnn=hidden_dim_cnn, hidden_gcn=hidden_gcn, out_dim=out_dim, training_len=training_len, num_nodes=rois, topk_ratio=topk_ratio, device=device)
